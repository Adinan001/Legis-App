...
class DashboardPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.card_movs    = CardMetrica("🔔", "Movimentações Novas",   "0", COLORS["accent"])
        for c in [self.card_proc, self.card_cli, self.card_vencido, self.card_hoje, self.card_semana, self.card_movs]:
        ...
        lbl_alertas = QLabel("🔄  Processos Monitorados (Datajud)")
        ...
        self.table_alertas.setHorizontalHeaderLabels(["Nº Processo", "Cliente", "Última Movimentação"])
    ...
    def atualizar_dados(self):
        # Movimentações novas dos processos monitorados
        try:
            from core.database import listar_processos_monitorados
            mons = listar_processos_monitorados()
            novos = sum(1 for m in mons if m.get("tem_novidade"))
            if hasattr(self, "card_movs"):
                self.card_movs.atualizar(str(novos))
        except Exception:
            pass
        ...
        # Processos monitorados (Datajud) com última movimentação
        try:
            from core.database import listar_processos_monitorados
            monitorados = listar_processos_monitorados()
        except Exception:
            monitorados = []
        self.table_alertas.setRowCount(len(monitorados))
        for i, p in enumerate(monitorados):
            # Formatar número CNJ
            num = p.get("numero_processo", "")
            num_fmt = num
            if len(num) == 20:
                num_fmt = f"{num[:7]}-{num[7:9]}.{num[9:13]}.{num[13:14]}.{num[14:16]}.{num[16:20]}"
            self.table_alertas.setItem(i, 0, QTableWidgetItem(num_fmt))
            self.table_alertas.setItem(i, 1, QTableWidgetItem(p.get("cliente", "")))
            mov = p.get("ultima_movimentacao", "") or "—"
            # Prefixo de novidade
            if p.get("tem_novidade"):
                mov = "🔴 " + mov
            item = QTableWidgetItem(mov)
            if p.get("tem_novidade"):
