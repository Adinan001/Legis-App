...
class DoutrinaPage(QWidget):
    ...
    def _nova_entrada(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                from core.ai.runner import rag_indexar_doutrina_async, analisar_tese_async
                novas = buscar_doutrina_entradas(self.tema_selecionado["id"])
                if novas:
                    nova = novas[0]
                    rag_indexar_doutrina_async(nova["id"], nova.get("autor",""),
                        nova.get("obra",""), nova.get("trecho",""),
                        self.tema_selecionado["tema"])
                    # Etapa 5: análise inteligente
                    texto_tese = f"{nova.get('autor','')}. {nova.get('obra','')}. {nova.get('trecho','')}"
                    analisar_tese_async("doutrina", nova["id"], texto_tese)
            except Exception:
                pass
