...
# ──────────────────────────────────────────────
# Diálogos de arquivo SEGUROS (não-nativos)
# Evitam crashes nativos do Windows com pastas sincronizadas (OneDrive)
# ──────────────────────────────────────────────
def dialogo_salvar(parent, titulo, nome_sugerido, filtro, pasta_inicial=None):
    """Abre diálogo de salvar não-nativo. Retorna o caminho ou None."""
    import os
    ...
    if not pasta_inicial:
        ...
        for cand in [os.path.join(pasta_inicial, "Documents"),
            ...
            if os.path.isdir(cand):
                ...
                break
    try:
        dlg = QFileDialog(parent, titulo)
        dlg.setOption(QFileDialog.Option.DontUseNativeDialog, True)
        dlg.setAcceptMode(QFileDialog.AcceptMode.AcceptSave)
        dlg.setDirectory(pasta_inicial)
        if nome_sugerido:
            dlg.selectFile(nome_sugerido)
        dlg.setNameFilter(filtro)
...
def dialogo_abrir(parent, titulo, filtro, pasta_inicial=None):
    """Abre diálogo de abrir não-nativo. Retorna o caminho ou None."""
    import os
    from PyQt6.QtWidgets import QFileDialog
    if not pasta_inicial:
        pasta_inicial = os.path.expanduser("~")
