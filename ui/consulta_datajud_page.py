...
class WorkerDatajud(QThread):
    ...
    def run(self):
        ...
        if len(num_puro) != 20:
            ...
            return
        # Consulta via subprocess (isola a rede do Qt, evita crash nativo no Windows)
        try:
            from core.ai.runner import consultar_datajud_subprocess
            ok, resultado = consultar_datajud_subprocess(num_puro, self.alias_tribunal, timeout=20)
            if ok:
...
class ConsultaDatajudPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Botão de cadastrar para monitoramento (aparece após consulta)
        acoes_row = QHBoxLayout()
        acoes_row.addStretch()
        self.btn_cadastrar = btn_primario("📌  Cadastrar para Monitoramento Automático")
        self.btn_cadastrar.setVisible(False)
        self.btn_cadastrar.clicked.connect(self.cadastrar_monitoramento)
        acoes_row.addWidget(self.btn_cadastrar)
        layout.addLayout(acoes_row)
    ...
    def iniciar_busca(self):
        ...
        # Limpar worker anterior com segurança
        if self.worker:
            try:
                self.worker.sucesso.disconnect()
                self.worker.erro.disconnect()
        ...
        self.worker.finished.connect(self._reabilitar_busca)
        self.worker.start()

    def _reabilitar_busca(self):
        try:
            from PyQt6 import sip
            if sip.isdeleted(self) or sip.isdeleted(self.btn_buscar):
                return
        except Exception:
            pass
        try:
    ...
    def _exibir_resultados(self, resultados):
        # Proteção: a página pode ter sido recriada enquanto o worker rodava
        try:
            from PyQt6 import sip
        ...
        try:
            ...
            self.btn_cadastrar.setVisible(True)
        except RuntimeError:
            return

    def cadastrar_monitoramento(self):
        from PyQt6.QtWidgets import QInputDialog, QMessageBox
        from core.database import cadastrar_processo_monitorado
        numero = self.txt_processo.text().strip()
        if not getattr(self, "_ultimos_resultados", None):
            return
        # Pedir o nome do cliente
        cliente, ok = QInputDialog.getText(self, "Vincular Cliente",
            "Nome do cliente vinculado a este processo:")
        if not ok:
            return
        trib_alias = self.cb_tribunal.currentData()
        ultima = self._ultimos_resultados[0]
        _id, novo = cadastrar_processo_monitorado(
            numero, trib_alias, cliente.strip(),
            ultima["movimento"], ultima["data_iso"])
        if novo:
            QMessageBox.information(self, "Processo cadastrado",
                f"✅ Processo cadastrado para monitoramento automático.\n\n"
                f"Ele aparecerá na aba de processos e será atualizado "
                f"automaticamente toda vez que você abrir o Legis.")
        else:
            QMessageBox.information(self, "Já cadastrado",
                "Este processo já estava cadastrado. Os dados foram atualizados.")
